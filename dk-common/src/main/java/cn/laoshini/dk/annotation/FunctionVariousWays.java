 ...
 * <p>
 * 表示某个功能可能具有多种实现，被标记的类就是其中一种实现，并且可以通过配置项来选择使用哪种实现方式（如果使用缺省实现，不需要在配置项中配置）；<br>
 * 所谓多种实现，意味着起码有一个基本的实现方式（作为缺省），因为某种原因不能满足需求，或不适合当前场景，所以选择更适合的实现方式
 * </p>
 * <p>
...
public @interface FunctionVariousWays {
    ...
    /**
     * 功能名称，作为区分其他实现的key（不区分大小写），用于用户选择实现方式；不指定则默认为是基本实现
     */
     ...
     * 实现类的实例是否是单例
     * <p>
     * 如果是单例对象，会把实例对象交给Spring容器管理；如果不是，则需要自己创建实例对象
     * </p>
     *
     * @return 是否是单例对象
     */

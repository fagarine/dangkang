...
class ColumnIdIncrementer implements IDbIdIncrementer {
    ...
    ColumnIdIncrementer(String columnName) {
        ...
        try {
            dataSource = SpringContextHolder.getBean("innerGameDataSource");
        } catch (BeansException e) {
            throw new DaoException("rdb.not.init", "没有连接关系数据库，无法通过数据库表实现id自增器");
        }

        DBTypeEnum dbType = SqlBuilder.getDBType();
        String tableName = DaConstant.ID_INCREMENTER_TABLE;
        switch (dbType) {
            case MYSQL:
